...
def load_model_and_encoders():
    try:
        model = tf.keras.models.load_model('predmodel.h5')
...
if model is None:
    st.error("⚠️ Model files not found. Please ensure 'predmodel.h5', 'gender_encoder.pkl', and 'scaler.pkl' are in the app directory.")
...
with input_col1:
    ...
    st.metric("**BMI**", f"{bmi:.1f}", 
              delta="Healthy" if 18.5 <= bmi <= 24.9 else "Check recommended range")
...
with dashboard_col1:
    ...
    st.metric("**Activity Level**", 
              "Highly Active" if steps > 10000 else "Active" if steps > 7500 else "Moderate" if steps > 5000 else "Sedentary",
              f"{steps:,} steps")
...
with dashboard_col2:
    ...
    st.metric("**Fitness Score**", 
              "Excellent" if exercise_hours > 5 else "Good" if exercise_hours > 3 else "Fair" if exercise_hours > 1 else "Poor",
              f"{exercise_hours} hrs/week")
    st.markdown('</div>', unsafe_allow_html=True)
